...
class SideBar : View {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for (i in b.indices) {
            paint.color = Color.rgb(125, 125, 125)//右侧字母颜色
            ...
            // 选中的状态
            if (i == choose) {
                paint.color = Color.parseColor("#ffffff")//选中颜色
                paint.isFakeBoldText = true

                val paint_new = Paint()
                paint_new.color = Color.parseColor("#ed7e00")//选中颜色
                val rect = Rect(singleHeight / 3, singleHeight * i + singleHeight / 4, singleHeight * 4 / 3, singleHeight * (i + 1) + singleHeight / 4)//左上右下
                val rectF = RectF(rect)
                canvas.drawRoundRect(rectF, singleHeight.toFloat(), singleHeight.toFloat(), paint_new)
            }
    ...
    /**listview 滑动监听   listview 和  右侧SideBar 联动 */
    fun setChoicePosition(letter: String) {
        var c = 0
        for (i in b.indices) {
            if (b[i] == letter) {
                c = i
